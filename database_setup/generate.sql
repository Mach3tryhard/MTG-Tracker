DROP TABLE SETURI CASCADE CONSTRAINTS;
DROP TABLE CARTI CASCADE CONSTRAINTS;
DROP TABLE PACHETE CASCADE CONSTRAINTS;
DROP TABLE LOCATII CASCADE CONSTRAINTS;
DROP TABLE JUCATORI CASCADE CONSTRAINTS;
DROP TABLE EVENIMENTE CASCADE CONSTRAINTS;
DROP TABLE MECIURI CASCADE CONSTRAINTS;
DROP TABLE CARTI_SETURI CASCADE CONSTRAINTS;
DROP TABLE PACHETE_CARTI CASCADE CONSTRAINTS;
DROP TABLE MECIURI_JUCATORI CASCADE CONSTRAINTS;
DROP TABLE PACHETE_MECIURI CASCADE CONSTRAINTS;
DROP VIEW V_CARTI_SI_EDITII CASCADE CONSTRAINTS;
DROP VIEW V_ANALIZA_PACHETE CASCADE CONSTRAINTS;
DROP VIEW V_PACHETE_GALERIE CASCADE CONSTRAINTS;
...
-- Vizualizarea compusa pentru editiile cartilor f
CREATE OR REPLACE VIEW V_CARTI_SI_EDITII AS
SELECT 
    c.NUME_CARTE,
    c.RARITATE,
    s.NUME_SET,
    s.DATA_LANSARE
FROM CARTI c
JOIN CARTI_SETURI cs ON c.NUME_CARTE = cs.NUME_CARTE
JOIN SETURI s ON cs.COD_SET = s.COD_SET;

-- Vizualizarea complexa pentru analiza pachetelor f
...
-- Vizualizarea pentru galeria de pachete care nu e cerinta din subiectul 3 dar e dragut
CREATE OR REPLACE VIEW V_PACHETE_GALERIE AS
SELECT 
    p.ID_PACHET,
    p.NUME_PACHET,
    p.FORMAT_JOC,
    (SELECT c.URL_IMAGINE 
     FROM CARTI c 
     JOIN PACHETE_CARTI pc ON c.NUME_CARTE = pc.NUME_CARTE 
     WHERE pc.ID_PACHET = p.ID_PACHET 
     AND ROWNUM = 1) AS URL_COPERTA
FROM PACHETE p;
